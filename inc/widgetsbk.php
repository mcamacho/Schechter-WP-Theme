<?php
/**
 * Makes a custom Widgets for displaying Image, Title, Resume and link - for the sidebar-home and sidebar-why
 *
 */
class Image_Brief_Link_Widget extends WP_Widget {
	 ...
	 * @return void
	 **/
	function Image_Brief_Link_Widget() {
		$widget_ops = array( 'classname' => 'widget_image_brief', 'description' => __( 'Use this widget to quick post image, title, text and link', 'twentyeleven' ) );
		$this->WP_Widget( 'widget_image_brief', __( 'Image Brief Link', 'twentyeleven' ), $widget_ops );
		$this->alt_option_name = 'widget_image_brief';
	...
	function widget( $args, $instance ) {
		$cache = wp_cache_get( 'widget_image_brief', 'widget' );
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Image Brief', 'twentyeleven' ) : $instance['title'], $instance, $this->id_base);
		...
		wp_cache_set( 'widget_image_brief', $cache, 'widget' );
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions['widget_image_brief'] ) )
			delete_option( 'widget_image_brief' );

		return $instance;
	}

	function flush_widget_cache() {
		wp_cache_delete( 'widget_image_brief', 'widget' );
	}
